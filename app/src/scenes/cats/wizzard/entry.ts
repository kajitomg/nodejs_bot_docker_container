...
interface CatsEntryProps {
  game: Game,
}

export const createEntryScene = composeWizardScene<CatsEntryProps>(
  async (ctx) => {
    const chatId = ctx.chat.id
    const game = GamesData.CATS
    ...
    try {
      ...
      console.error(new HandlerError(400, 'Ошибка: Меню Cats', e))
    }
