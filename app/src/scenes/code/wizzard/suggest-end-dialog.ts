...
interface CodeSuggestEndDialogProps {
  game: Game,
...
export const createGiveCodeEndDialogScene = composeWizardScene<CodeSuggestEndDialogProps>(
  async (ctx) => {
    const game = ctx.scene.session?.props?.game
    
    const markup = Markup.inlineKeyboard(
      [
        Markup.button.callback(ctx.i18n.t('code_suggest.buttons.back_to',{ menu_name: ctx.i18n.t('game.name',{ game_name: game.name }) }), 'back_to'),
    ...
    const text = genMessage({
      header: genMessage({
        ...
        body: italic(ctx.i18n.t(`code_suggest.data.warning_add_to_db.${ctx.scene.session?.props.result}`)),
      }),
