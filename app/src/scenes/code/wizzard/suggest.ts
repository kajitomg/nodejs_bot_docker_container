  ...
  async (ctx) => {
    ...
    const markup = Markup.inlineKeyboard(
      [
        ...
        Markup.button.callback(ctx.i18n.t('code_suggest.buttons.back'), nextSceneHandler.create(ctx.wizard.state.options.entry)),
  ...
  async (ctx, done) => {
    ...
    if (callback_data) {
      nextSceneHandler.on(callback_data, async (value) => {
        ...
        if (value === types.GIVE_CODE_ADD_TO_DB && (!ctx.wizard.state.code_name || !ctx.wizard.state.code_content)) {
