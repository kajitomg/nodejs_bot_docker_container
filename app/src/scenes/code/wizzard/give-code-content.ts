...
export const createGiveCodeContentScene = composeWizardScene(
  async (ctx) => {
    ...
    const markup = Markup.inlineKeyboard(
      [
        Markup.button.callback(ctx.i18n.t('code.giveContent.buttons.back'), createNextScene(types.GIVE_CODE)),
      ],{ columns: 2 }
    )
    
    const text = genMessage({
      header: fmt(ctx.i18n.t('code.giveContent.header')),
      body: fmt(fmt(`- ${ctx.i18n.t('code.giveContent.content')}${ctx.wizard.state.code_content ? '' : '*'}: `), bold(ctx.wizard.state.code_content ? ctx.wizard.state.code_content : '-')),
    })
