...
interface HotEntryProps {
  game: Game,
}

export const createEntryScene = composeWizardScene<HotEntryProps>(
  async (ctx) => {
    const chatId = ctx.chat.id
    const game = GamesData.HOT
    ...
    try {
      ...
      console.error(new HandlerError(400, 'Ошибка: Меню HOT Wallet', e))
    }
