...
interface BumsEntryProps {
  game: Game,
}

export const createEntryScene = composeWizardScene<BumsEntryProps>(
  async (ctx) => {
    const chatId = ctx.chat.id
    const game = GamesData.BUMS
    ...
    try {
      ...
      console.error(new HandlerError(400, 'Ошибка: Меню Bums', e))
    }
