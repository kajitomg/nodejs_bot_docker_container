...
interface PawsEntryProps {
  game: Game,
}

export const createEntryScene = composeWizardScene<PawsEntryProps>(
  async (ctx) => {
    const chatId = ctx.chat.id
    const game = GamesData.PAWS
    ...
    try {
      ...
      console.error(new HandlerError(400, 'Ошибка: Меню Paws', e))
    }
