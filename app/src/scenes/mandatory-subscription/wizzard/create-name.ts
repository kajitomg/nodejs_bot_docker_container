...
interface MandatoryCreateNameProps {
  channel: Partial<Omit<IMandatoryChannel, 'id'>>
}

export const createCreateMandatoryChannelNameScene = composeWizardScene<MandatoryCreateNameProps>(
  async (ctx) => {
    try {
      ...
      await sendMessage(ctx, {
        text: fmt(
          ...
          italic(`Название${ctx.scene.session.props.channel?.name ? '' : '*'}: ${ctx.scene.session.props.channel.name || '-'}`),'\n\n',
