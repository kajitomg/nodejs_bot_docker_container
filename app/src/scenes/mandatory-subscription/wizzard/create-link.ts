...
export const createCreateMandatoryChannelLinkScene = composeWizardScene(
  async (ctx) => {
    try {
      ...
      const message = await send(ctx, fmt(
        ...
        italic(`Ссылка${ctx.wizard.state?.create_mandatory_channel?.link ? '' : '*'}: ${ctx.wizard.state.create_mandatory_channel.link || '-'}`),'\n\n',
  ...
  async (ctx, done) => {
    ...
    try {
      ...
      if (callback_query) {
        ...
        if (!ctx.wizard.state.create_mandatory_channel.name) {
          ctx.wizard.state.nextScene = types.CREATE_NAME;
        } else if (!ctx.wizard.state.create_mandatory_channel.id) {
          ctx.wizard.state.nextScene = types.CREATE_ID;
        } else {
          ctx.wizard.state.nextScene = types.CREATE;
        }
        ctx.wizard.state.create_mandatory_channel.link = message_text;
      }
