...
interface HamsterKombatEntryProps {
  game: Game,
}

export const createEntryScene = composeWizardScene<HamsterKombatEntryProps>(
  async (ctx) => {
    const chatId = ctx.chat.id
    const game = GamesData.HAMSTERKOMBAT
    ...
    try {
      ...
      const markup = Markup.inlineKeyboard(
        [
          Markup.button.url(ctx.i18n.t('game.buttons.bot_for_gencodes'), 'https://t.me/hamtabor_bot'),
      ...
      console.error(new HandlerError(400, 'Ошибка: Меню Hamster Kombat', e))
    }
